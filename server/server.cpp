...
Game::Game(Player *white, Player *black, int id): id(id), white(white), black(black), board(), moves(), finished(false), score(0) {
  board_from_fen_str(&board, "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1");
}
...
void Game::serialize(std::ostream& out) {
  ...
  out << "game " << id << ", " << white->id << ", " << black->id << ", " << board_str << ",";
  for(move move: moves) {
    char move_str[5];
...
void Player::serialize(std::ostream& out, const std::string& api_key, bool do_api_key) {
  out << "player " << id << ", ";
  if(do_api_key) {
    out << api_key << ", ";
  } else {
...
int main() {
  ...
  uWS::App().ws<PerSocketData>("/*", {
    ...
    .message = [](auto *ws, std::string_view message, uWS::OpCode opCode) {
      ...
      if(cmd[0] == "apikey") {
        if(cmd.size() < 2) {
          ws->send("error expected 1 argument to command apikey", opCode);
      ...
      // command: observe
      // register as a game observer, and get sent the current state of the game
      ...
      // set the current player's name
        ...
        if(cmd.size() < 2) {
          ws->send("error expected 1 argument to command name", opCode);
      ...
      // create a new game and return api keys
        ...
        // send api keys back
        ws->send("newgame " + std::to_string(game->id) + ", " + std::to_string(res.first.first->id) + ", " + res.first.second + ", " +  std::to_string(res.second.first->id) + ", " + res.second.second + "\n", opCode);
      ...
      // make the move on the last sent position
        ...
        auto player = ((PerSocketData *)ws->getUserData())->player;
        if(cmd.size() < 2) {
          ws->send("error expected 1 argument to command move", opCode);
        } else if(player == nullptr) {
          ws->send("error not registered as a player (use apikey command)", opCode);
        } else {
